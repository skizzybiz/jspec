...
;(function(){
  ...
  JSpec = {
    ...
    // --- Utilities
    // These will be available as local functions, without having to namespace
    // them with this or JSpec.
    utilities: {
      ...
      enqueue : function(f, module, args, errorMessage) {
        if (false) {
          console.log("Trying to apply function: " + f)
          console.log(module)
        }
        try { return f.apply(module, args) }
    ...
    include : function(object) {
      ...
      if ('matchers' in module) addMatchers(module.matchers)
    ...
    evalHook : function(module, name, args) {
      ...
      var f = new Function(params.join(), 'with (this.utilities) { with (context) { with (JSpec) {  ' + contents + ' }}}')
      var errorMessage = 'Error in hook ' + module.name + "." + name + ': '
      return enqueue(f, module, args, errorMessage)
    },
    ...
    evalBody : function(body, errorMessage) {
      var o = {}
      o.dsl = this.DSL || this.DSLs.snake
      o.matchers = this.matchers
      o.context = this.context || this.defaultContext
      o.utilities = this.utilities
      var contents = this.contentsOf(body)
      
      hook('evaluatingBody', o.dsl, o.matchers, o.context, contents)
      f = new Function([], 'with(this.utilities){ with (this.dsl){ with (this.context) { with (this.matchers) { ' + contents + ' }}}}')
      enqueue(f, o, [], errorMessage)
    },
  ...
  var main = this
  var find = JSpec.utilities.any
  for (util in JSpec.utilities) {
    eval('var ' + util + ' = JSpec.utilities.' + util)
    JSpec[util] = JSpec.utilities[util]
  }
